...
for(i in 1:length(MaxCorr)){	
	MaxCorr[i]												#tests correlation for each sample; if below .98, adds to list of fails
...
if(!is.null(exon_numbers)&any(Exon!="All")){
    ...
    Clinical_Numbering=unlist(lapply(Gene, function(t){ a <- ifelse(t != "All", unlist(strsplit(t, "_"))[2], "NA")}))
