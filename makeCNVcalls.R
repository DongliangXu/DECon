...
temp = cnv.calls[cnv.calls$sample==Sample,]
	    ...
	    name <- unlist(lapply(Gene, function(t){  unlist(strsplit(t, "_"))[1]  }))
            exon <- unlist(lapply(Gene, function(t){  unlist(strsplit(t, "_"))[2]  }))
            
            gene_names <- unique(name)
            for(i in gene_names) {
		pdf(file=paste(plotFolder,"/",Sample,"_", i, "_", paste(exon[name == i],collapse="_"),"_",which(cnv_genes_sample_index==call_index),".pdf",sep=""),useDingbats=F)
